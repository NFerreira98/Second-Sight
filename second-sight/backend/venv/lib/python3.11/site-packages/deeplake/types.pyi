...
class IndexType:
    ...
    Attributes:
        ...
        Unique: A unique index that enforces column-level uniqueness at commit time.
    """
    ...
    Unique: typing.ClassVar[IndexType]  # value = <IndexType.Unique: 7>
    __members__: typing.ClassVar[dict[str, IndexType]]  # value = {'Inverted': <IndexType.Inverted: 1>, 'BM25': <IndexType.BM25: 2>, 'Exact': <IndexType.Exact: 3>, 'PooledQuantized': <IndexType.PooledQuantized: 4>, 'Clustered': <IndexType.Clustered: 5>, 'ClusteredQuantized': <IndexType.ClusteredQuantized: 6>, 'Unique': <IndexType.Unique: 7>}
    def __eq__(self, other: typing.Any) -> bool:
        ...
    def __getstate__(self) -> int:
        ...
...
class UniqueIndex:
    """
    Represents a unique column index type.

    Used to create indexes that enforce column-level uniqueness at commit time.
    """
    __hash__: typing.ClassVar[None] = None
    @typing.overload
    def __init__(self) -> None:
    ...
    def __init__(self, type: IndexType) -> None:
        ...
    def __init__(self, type: IndexType | None = None) -> None:
        ...
    def __str__(self) -> str:
        ...
...
class Index:
    ...
    def __eq__(self, other: Index) -> bool:
        ...
    def __init__(self, index_type: TextIndex | EmbeddingIndexType | EmbeddingsMatrixIndexType | JsonIndex | NumericIndex | UniqueIndex) -> None:
        ...
...
class Type:
    ...
    def is_serial(self) -> bool:
        """
        Returns:
            bool: True if this type is a serial (auto-increment) type, False otherwise.
        """
        ...
...
class TypeKind:
    ...
    Serial: typing.ClassVar[TypeKind]
...
"""
A unique index that enforces column-level uniqueness at commit time.

This index ensures that no two rows in the column have the same value.
Uniqueness is validated during commit, not during insertion, to avoid contention during parallel ingest.
"""
...
def Nullable(type: DataType | str | Type) -> Type:
    """
    Wraps another type to allow null (None) values.

    A nullable column stores an additional boolean flag per element indicating
    whether the value is null. This is useful when data may be missing or
    not yet available.

    Parameters:
        type: DataType | str | Type
            The inner type to wrap. Can be any data type, string shorthand,
            or Type. Cannot wrap another Nullable.

    Returns:
        Type: A new nullable type.
    ...
    Examples:
        Create a nullable column and append data with None values:
        ```python
        ds.add_column("nullable_int", types.Nullable(types.Int32()))
        ds.append({"nullable_int": [1, None, 3, None, 5]})
        ```

        Query for null values using TQL:
        ```python
        results = ds.query("SELECT * WHERE nullable_int IS NULL")
        results = ds.query("SELECT * WHERE nullable_int IS NOT NULL")
        ```
    """
    ...
...
def Serial(dtype: DataType | str = "int64") -> Type:
    """
    Creates a serial (auto-increment) column type.

    Serial columns automatically generate unique, monotonically increasing integer IDs
    on each append. Values cannot be manually provided or updated.

    Parameters:
        dtype: DataType | str
            The underlying integer data type. Must be an integer type.
            Defaults to "int64". Supported: "int16", "int32", "int64".

    Returns:
        Type: A new serial data type.
    ...
    Examples:
        ...
        ```python
        ds.add_column("id", types.Serial())
        ds.add_column("id32", types.Serial("int32"))
        ```
    """
    ...
