...
class Tag:
    ...
    def query(self) -> str | None:
        """
        The TQL query string if this tag was created from a query view, None otherwise
        """
        ...
...
class Column(ColumnView):
    ...
    def __setitem__(self, index: int | slice, value: typing.Any) -> None:
        ...
        """
        ...
    def create_index(self, index_type: str | types.IndexType | types.TextIndex | types.JsonIndex | types.NumericIndex | types.UniqueIndex | types.EmbeddingIndexType | types.EmbeddingsMatrixIndexType | types.Index) -> None:
        """
    ...
    def drop_index(self, index_type: str | types.IndexType | types.TextIndex | types.JsonIndex | types.NumericIndex | types.UniqueIndex | types.EmbeddingIndexType | types.EmbeddingsMatrixIndexType | types.Index) -> None:
        """
...
class CompactionMode:
    """
    Enumeration of available compaction modes in deeplake.

    Members:
        Automatic: Compact when deeplog thresholds are met (default behavior).
        Off: Compaction is disabled during commit; use compact() explicitly.
    """
    Automatic: typing.ClassVar[CompactionMode]
    Off: typing.ClassVar[CompactionMode]
    __members__: typing.ClassVar[Dict[str, CompactionMode]]
...
class Dataset(DatasetView):
    ...
    """
    The compaction mode of the dataset. Controls whether compaction runs automatically during commit.

    Examples:
        ```python
        ds.compaction_mode = deeplake.CompactionMode.Off  # disable auto-compaction
        ds.commit()  # fast commit without compaction
        ds.compact()  # compact explicitly when ready
        ```
    """
    ...
    def compact(self) -> None:
        """
        Triggers compaction independently of commit. Merges trailing small commits into a single compacted commit.

        The dataset must not have uncommitted changes.

        Examples:
            ```python
            ds = deeplake.create("tmp://")
            ds.add_column("col", "int32")
            ds.compaction_mode = deeplake.CompactionMode.Off
            ds.append({"col": [1, 2, 3]})
            ds.commit()
            ds.compact()
            ```
    ...
    def compact_async(self) -> FutureVoid:
        """
        Asynchronously triggers compaction. See [deeplake.Dataset.compact][] for more information.
        """

    def build_indexes(self) -> None:
        """
        Triggers index building independently of commit. Builds any pending indexes.
    ...
    def build_indexes_async(self) -> FutureVoid:
        """
        Asynchronously triggers index building. See [deeplake.Dataset.build_indexes][] for more information.
        """
